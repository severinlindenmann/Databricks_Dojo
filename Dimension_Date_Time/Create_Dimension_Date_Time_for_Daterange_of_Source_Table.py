...
# MAGIC %md
# MAGIC # Create Dimensions for Date and Time for a given Table
# MAGIC
# MAGIC ## Description  
...
sql = f"""
CREATE OR REPLACE VIEW {catalog}.silver.vw_dim_date_minute_{source_table.replace(".", "_")} AS
  SELECT
    dd.Date,
    dd.DatePK,
    dd.DateString,
  ...
  LEFT OUTER JOIN {catalog}.silver.dim_mapping_minute_{source_table.replace(".", "_")} AS mm ON dd.DatePK = mm.DateUtcFK
  LEFT OUTER JOIN {catalog}.silver.dim_time_minute_{source_table.replace(".", "_")} AS tm ON mm.WeekTimePK = tm.WeekTimePK
"""
